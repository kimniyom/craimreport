...
class Upload_txtController extends Controller {

    public function actionUploadfile() {
        $this->render("//Upload_txt/form_uploads");
    }

    public function actionImport_txt() {
    ...
    public function actionImport() {
        ...
        $i = 0;
        $objFopen = fopen("./upload_csv/" . $file, "r");
        if ($objFopen) {
            while (!feof($objFopen)) {
                $datas = fgets($objFopen, 4096);
                //echo $file . "<br>";
                if (substr(trim($datas), 0, 2) == "*|") {
                    $i++;
                    $d[] = $datas;
                }
            }
